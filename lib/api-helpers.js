...
const API_BASES = (process.env.STREAM_API_BASES || 'https://streamed.pk/api')
    ...
    .map(base => base.trim())
    .filter(Boolean);
const DEFAULT_USER_AGENT = process.env.USER_AGENT || 'SportsViewer/1.0';
...
const normalizeScoreboardTeam = competitor => {
    if (!competitor) return null;
    const team = competitor.team || {};
    const name = team.displayName || team.shortDisplayName || team.name || '';
    const logo = team.logo || selectLogo(Array.isArray(team.logos) ? team.logos : []);
    const rawScore = competitor.score ?? competitor?.score?.displayValue ?? competitor?.score?.value ?? null;
    const score = rawScore === '' ? null : rawScore;
    if (!name && !logo && score === null) return null;
    return {
        id: team.id || null,
        name: name || null,
        shortName: team.shortDisplayName || team.abbreviation || name || null,
...
const buildGamesFromScoreboard = (events, league) => {
    const now = Date.now();
    const games = (events || []).map(event => {
        ...
        const startDate = competition.startDate || event.date || null;
        const timestamp = startDate ? new Date(startDate).getTime() : null;
        const safeTimestamp = Number.isNaN(timestamp) ? null : timestamp;
        const isUpcoming = !isLive && !isEnded && safeTimestamp && safeTimestamp > now;

        const title = event.name || event.shortName || (
            awayTeam?.name && homeTeam?.name ? `${awayTeam.name} vs ${homeTeam.name}` : 'Matchup'
        );
        const id = event.id || competition.id || null;
        const slugBase = id ? `espn-${sanitizeSlug(id)}` : `espn-${sanitizeSlug(title) || Date.now()}`;
        ...
        return {
            ...
            gameTime: safeTimestamp ? new Date(safeTimestamp).toISOString() : null,
            timestamp: safeTimestamp,
            isLive,
            isUpcoming,
            isEnded,
            isPopular: Boolean(event?.competitions?.[0]?.featured || event?.featured),
            sources: [],
            currentSource: null,
            source: 'scoreboard',
    ...
    for (const base of API_BASES) {
        try {
            ...
            const headers = origin
                ...
                : undefined;
            const data = await fetchJson(url, { retries: FETCH_RETRIES, headers });
